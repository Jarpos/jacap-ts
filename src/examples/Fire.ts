...
export namespace Fire {
    ...
    export const Update = (current: States, neighbors: Neighborhood<States>) => {
        switch (current) {
            case States.Tree:
                return catchesFire(.1, neighbors) ? States.TreeFire : States.Tree;
            case States.Grass:
                return catchesFire(.15, neighbors) ? States.GrassFire : States.Grass;

            case States.GrassFire:
                return neighbors.GetMooreCount(
                    (v) => Utility.isAnyOf(v,
                        States.TreeFire, States.GrassFire,
                        States.BurnedGrass, States.BurnedTree)) === 8
            ...
            case States.BurnedGrass:
                return States.BurnedGrass;
            case States.BurnedTree:
                return States.BurnedTree;
    ...
    function catchesFire(likelihood: number, neighbors: Neighborhood<States>) {
        ...
        return ((count / 10) * likelihood) > Math.random();
    }
}
